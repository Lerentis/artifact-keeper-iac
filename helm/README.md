...
helm install my-artifact-keeper ./helm
```
...
| web | object | `{"affinity":{},"enabled":true,"env":{"NEXT_PUBLIC_API_URL":"","NODE_ENV":"production"},"image":{"pullPolicy":"Always","repository":"ghcr.io/artifact-keeper/artifact-keeper-web","tag":"dev"},"nodeSelector":{},"replicaCount":1,"resources":{"limits":{"cpu":"1","memory":"1Gi"},"requests":{"cpu":"250m","memory":"256Mi"}},"service":{"port":3000,"type":"ClusterIP"},"tolerations":[],"topologySpreadConstraints":[]}` | Next.js web frontend |
...
# Template and verify
helm template my-artifact-keeper helm/ --debug

# Dry-run install
helm install my-artifact-keeper helm/ --dry-run --debug

# Install in a test namespace
kubectl create namespace artifact-keeper-test
helm install my-artifact-keeper helm/ -n artifact-keeper-test
```
...
3. Make your changes to the `helm/` directory
4. **Bump the version** in `helm/Chart.yaml` (required for releases)
5. Run `helm-docs helm/` to update the README
